  ...
  getMyTasks: async (req, res) => {
    try {
      ...
      const rows = await queryAsync(
        `SELECT
         ${selectParts.join(', ')},
         GROUP_CONCAT(ua._id ORDER BY ua._id) AS assigned_user_ids,
         GROUP_CONCAT(ua.public_id ORDER BY ua._id) AS assigned_user_public_ids,
         GROUP_CONCAT(ua.name ORDER BY ua._id) AS assigned_user_names,
         GROUP_CONCAT(COALESCE(ta_all.is_read_only, 0) ORDER BY ua._id) AS assigned_user_read_only
       FROM tasks t
