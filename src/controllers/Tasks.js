  ...
  try {
    ...
    try {
      ...
      if (newStatus === 'APPROVE') {
        ...
        if (finalNewAssigneeId) {
          // the new assignee ALWAYS gets write access (read_only = 0)
          const newIsReadOnly = 0;
          console.log('[REASSIGN DEBUG] Inserting NEW assignee:', { userId: finalNewAssigneeId, is_read_only: newIsReadOnly });
          await new Promise((resolve, reject) => {
            connection.query(
        ...
        // The old assignee (who requested reassignment) gets read-only access (is_read_only = 1)
        console.log('[REASSIGN DEBUG] Inserting OLD assignee (read-only):', { userId: resignRequest.requested_by, is_read_only: 1 });
        await new Promise((resolve, reject) => {
          connection.query(
            'INSERT INTO taskassignments (task_Id, user_Id, is_read_only) VALUES (?, ?, 1)',
