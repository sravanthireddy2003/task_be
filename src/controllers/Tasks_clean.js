...
router.get('/', ruleEngine(RULES.TASK_VIEW), requireRole(['Admin','Manager','Employee']), async (req, res) => {
  try {
    const rows = await new Promise((resolve, reject) => db.query('SELECT id, public_id, title, status FROM tasks ORDER BY id DESC LIMIT 100', (e, r) => e ? reject(e) : resolve(r)));
    return res.json({ success: true, data: rows || [] });
  } catch (e) {
    logger.error('tasks:list error: ' + (e && e.message));
...
router.post('/createjson', ruleEngine(RULES.TASK_CREATE), requireRole(['Admin','Manager']), async (req, res) => {
  try {
    ...
    const result = await new Promise((resolve, reject) => db.query('INSERT INTO tasks (title, description, client_id, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?)', [title, description || null, client_id || null, createdAt, createdAt], (e, r) => e ? reject(e) : resolve(r)));
    ...
    logger.error('tasks:create error: ' + (e && e.message));
...
router.post('/selected-details', ruleEngine(RULES.TASK_VIEW), requireRole(['Admin','Manager','Employee']), async (req, res) => {
  try {
    ...
    const rows = await new Promise((resolve, reject) => db.query('SELECT id, public_id, title, status FROM tasks WHERE id IN (?) OR public_id IN (?)', [numeric.length ? numeric : [-1], ids], (e, r) => e ? reject(e) : resolve(r)));
    return res.json({ success: true, data: rows || [] });
  } catch (e) {
    logger.error('selected-details error: ' + (e && e.message));
